...
use OneSignal;
class cmd_hour extends Command
{
    /**
    ...
    {
        if(live_tournaments::Where('game_time',0)->where('status',0)->exists()){
            $live_tournaments = live_tournaments::Where('game_time',0)->where('status',0)->get();
...
/*                         $params = [];
        ...
        if(tournaments::Where('game_time',0)->where('status',0)->exists()){
            $tournaments = tournaments::Where('game_time',0)->where('status',0)->where('type',0)->orWhere('type',1)->get();
    ...
    // public function sendPush($title,$msg,$id){
    //     $params = [];
    //     $params['headings'] = [ "en"=>$title ];
    //     OneSignal::addParams($params)->sendNotificationToUser($msg,$id);
    // }
}
